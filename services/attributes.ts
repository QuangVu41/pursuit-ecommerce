...
import { generateOrQueryForSearch, hasDuplicates } from '@/lib/helpers';
import { deleteFromS3 } from '@/lib/upload';
import { AttrFormSchemaType } from '@/schemas/attributes';
...
export const createAttribute = async (data: AttrFormSchemaType) => {
  const user = await getUserSession();
  const userId = user?.id;
  if (!userId) throw new ExpectedError('Unauthenticated!');
...
export const getAllUserFilteredAttributes = async (searchParams: { [key: string]: string }) => {
  const user = await getUserSession();
  const userId = user?.id;
  if (!userId) throw new ExpectedError('Unauthenticated!');
...
export const updateAttribute = async (data: AttrFormSchemaType) => {
  if (data.mode === 'edit') {
    const user = await getUserSession();
    const userId = user?.id;
    if (!userId) throw new ExpectedError('Unauthenticated!');
    ...
    if (!id) throw new ExpectedError('Attribute ID is required for editing!');
...
export const deleteManyAttributes = async (ids: string[]) => {
  ...
  await Promise.all(ids.map(deleteAttribute));
};
